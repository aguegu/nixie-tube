...
public:
	...
	DigitalTubeDisplay(uint8_t * pin_digit, uint8_t digit_length, uint8_t pin_A = 3, uint8_t pin_B = 13, uint8_t pin_C = 10, uint8_t pin_D = 6, uint8_t pin_E = 5, uint8_t pin_F = 4, uint8_t pin_G = 8, uint8_t pin_DP = 7);
	virtual ~DigitalTubeDisplay();

	void turnOff();
	void turnOn();

	void setDelaySpan(uint16_t span = 0x1000);
...
private:
	const byte _digit_length;
	const uint8_t *_pin_digit;
	uint8_t _pin_segment[8];
	uint16_t _delaySpan;
	byte * _cache;
